...
if (isset($_POST['changepwd'])) { 
	...
	if (empty($_POST['password'])||empty($_POST['pwd2'])||empty($_POST['pwd2confirm'])){
		header("location: ../users/resetpwd.php?error=Please fill in the above fields");
		exit();
	}
	
	else{
        if($_POST['pwd2']!== $_POST['pwd2confirm']){
            header("location: ../users/resetpwd.php?error=The passwords do not match");
            exit();
        }
        elseif(($_POST['password']== $_POST['pwd2'])||($_POST['password']== $_POST['pwd2confirm'])){
          header("location: ../users/resetpwd.php?error=The old password and new password should not be the same");
          exit();
        }
            else{
    ...
    $date=date("Y-m-d h:i:sa");
		$sql = "SELECT password FROM tenants WHERE username= '".$activeuser."'"; 
		...
		if($row=mysqli_fetch_array($result)){
		  ...
		  if(password_verify($pwd1,$row['password'])){
            $pwd2x= password_hash($pwd2,PASSWORD_DEFAULT);
              $sql2="UPDATE tenants SET password='".$pwd2x."',update_date='".$date."' WHERE username='".$activeuser."'";
              if(mysqli_query($conn, $sql2))
              {
              	header("location: ../users/resetpwd.php?error=Update Success");
			          exit();
              }
              else{	  
                header("location: ../users/resetpwd.php?error=Something went wrong please try again".mysqli_error($conn));
                exit();
                
              } 
		  }
		  else{	  
			header("location: ../users/resetpwd.php?error=Incorrect details");
			exit();
		  ...
		  header("location: ../users/resetpwd.php?error=Please Sign Up Or Log In If you have an account");
		  exit();
		}
